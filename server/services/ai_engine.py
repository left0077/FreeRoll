...
## 核心规则（违反将导致游戏崩溃）
1. 需要判定时，调用 roll_dice 函数。绝对禁止在叙事中自己写"d20=10"之类的文字——你没有权限掷骰，必须通过函数调用
2. 数值变化写在 <controls> 块里，禁止在叙事中写"HP:15→12"
3. 叙事只写故事，不写任何游戏机制
4. 每次回复必须以 <narrative> 开头
...
def build_messages(room: dict, player_input: str, player_character_name: str) -> list:
    ...
    if wm:
        ...
        if c.get("style_desc"):
            ...
            style_hints.append(f"文风：所有叙事必须严格使用{c['style']}风格")
        if c.get("tone"): style_hints.append(f"主线紧密度：{c['tone']}")
        if c.get("custom_style"): style_hints.append(c["custom_style"])
        if style_hints:
            messages.append({"role": "system", "content": "【创作约束 — 每次叙事必须严格遵守，不得遗忘】" + "；".join(style_hints)})
